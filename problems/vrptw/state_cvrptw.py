...
class StateCVRPTW(NamedTuple):
    ...
    def initialize(input, visited_dtype=torch.uint8):
        ...
        return StateCVRPTW(
            ...
            total_service_times=torch.zeros(batch_size, 1, device=loc.device),
            total_delay_times=torch.zeros(batch_size, 1, device=loc.device),
            total_early_times=torch.zeros(batch_size, 1, device=loc.device),
    ...
    def update(self, selected):
        ...
        total_service_times = self.total_service_times + self.service_times[self.ids, selected]
        delay = self.time_window_finish[self.ids, selected] - cur_time
        early = cur_time - self.time_window_start[self.ids, selected]
        total_delay_times = self.total_delay_times + delay*(delay > 0).float()
        total_early_times = self.total_early_times + early*(early > 0).float()
        ...
        return self._replace(
            ...
            lengths=lengths, cur_coord=cur_coord, i=self.i + 1, total_service_times=total_service_times,
            total_delay_times=total_delay_times, total_early_times=total_early_times
        )
